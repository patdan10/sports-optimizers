...
def csvScrape():
    ...
    starters = wfscrape.runner("S04", "LEV", "ben")
    print(starters)
    ...
    with open(loc, encoding='mac_roman') as csvfile:
        ...
        for row in reader:
            ...
            if row[9] in hardPlayers:
                row[9] = hardPlayers[row[9]]
            
            if row[9] not in starters:
                print(row[9])
                continue
    ...
    if (len(utilPlayers) != 22):
        print("Length wrong! Length Listo: " + str(len(utilPlayers)))
        quit()
